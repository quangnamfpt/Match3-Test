...
namespace Utilities
{
	public static class SimplePool
	{
		...
		class Pool
		{
			...
			public T Spawn<T>(Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false, string Tag = null)
			{
				GameObject obj;
				if (inactive.Count == 0)
				{
		...
		public static void Preload(GameObject prefab, Transform parent, int qty = 1)
		{
			...
			for (int i = 0; i < qty; i++)
			{
				objs[i] = Spawn(prefab, Vector3.zero, Quaternion.identity, parent);
			}
		...
		public static GameObject Spawn(GameObject prefab, Vector3 pos) => Spawn(prefab, pos, Quaternion.identity);
		...
		public static TKey Spawn<TKey>(GameObject prefab, Vector3 pos, Quaternion rot)
		{
			Init(prefab);
			return pools[prefab].Spawn<TKey>(pos, rot);
		}
		public static TKey Spawn<TKey>(GameObject prefab, Vector3 pos) => Spawn<TKey>(prefab, pos, Quaternion.identity);
		/// <summary>
		/// 
		/// </summary>
		...
		/// <returns></returns>
		public static T Spawn<T>(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false, string tag = null)
		{
			Init(prefab);
			return pools[prefab].Spawn<T>(pos, rot, parent, isLocal, tag);
		}
		public static GameObject Spawn(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent, bool isLocal = false)
		{
			Init(prefab);
